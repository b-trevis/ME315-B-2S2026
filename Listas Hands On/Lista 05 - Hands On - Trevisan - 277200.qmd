---
title: "Lista 05 - Hands On - Trevisan - 277200"
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS NOVA_TAXA
FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(*) AS QNTD_FUNC_DNO20
FROM EMPLOYEE
WHERE DNO = 20
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS QNTD_DEPT
FROM EMPLOYEE
```
